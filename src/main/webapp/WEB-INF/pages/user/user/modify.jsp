...
<body style="min-width: 540px; overflow: auto; overflow: hidden">
					...
					<tr>
						...
						<td>		<select name="orgId" id="organ" class="formText" dep="${user.depId}" >
									...
									<c:forEach items="${orgList }" var="org">
										<option value="${org.id }"  <c:if test="${user.orgId==org.id }"> selected="selected" </c:if>  >${org.name }</option>
									</c:forEach>
								</select></td>
					...
					<th>职位:</th>
						<td>		<select name="depId" id="department" class="formText">
									...
									<c:forEach items="${departList }" var="depart">
										<option value="${depart.id }"  <c:if test="${user.depId==depart.id }"> selected="selected" </c:if>  >${depart.name }</option>
									</c:forEach>								
								</select></td>
						<td><span style="color:red;font-size:20px">*</span></td>
						<th>登录密码:</th>
	...
	<script type="text/javascript" src="<%=basePath%>pageJs/user/user/modify.js?vs=<%=System.currentTimeMillis()%>"></script>
</body>
</html>
